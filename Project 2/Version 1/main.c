...
void createQueryFromHead(int* queryArray, int head){
    for(int i=0;i<4;++i){
        queryArray[i]=head+i;
    }
}

void createQuery(int* queryArray, int a, int b, int c, int d){
    queryArray[0]=a;
    queryArray[1]=b;
    queryArray[2]=c;
    queryArray[3]=d;
...
//note: experiment with recursion!
int mysub(int n){
    int explored = 0;
    int typeA = 0;
    int headOfFours = 0;
    ...
    while(current <= n-3){
        createQueryFromHead(query, current);
        int result = QCOUNT(1,query);
        current += 4;
        switch (result) {
            case 4:
                //
                //  all four that we just found are the same
                //
                if(headOfFours == 0){
                    headOfFours = current;
                    explored += 4;
                    ...
                    //
                    //  evaluate two fours together
                    //
                    createQuery(query, headOfFours, headOfFours+1, spareFour, current);
                    int subResult = QCOUNT(1, query);
                    explored += 8;
                    switch (subResult) {
                        case 0:
                            break;
            ...
            case 2:
                //
                //  just save the twos with a spare element
                //
                twos[twosIndex++] = current;
                current++;
                break;
    ...
    //handle the spares last, but remember they are there
